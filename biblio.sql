...
-- Créer un déclencheur (trigger) qui ajuste la séquence lors de l'insertion d'une nouvelle nationalité
DELIMITER //
CREATE TRIGGER before_insert_nationalite
BEFORE INSERT ON nationalite
FOR EACH ROW
BEGIN
    DECLARE max_num INT;
    
    -- Trouver le numéro maximum existant dans la table
    SELECT MAX(num) INTO max_num FROM nationalite;
    
    -- Réinitialiser la séquence pour commencer au numéro précédent
    SET NEW.num = IFNULL(max_num, 0) + 1;
END;
//
DELIMITER ;
